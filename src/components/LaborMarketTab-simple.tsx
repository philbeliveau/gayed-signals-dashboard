...
export default function LaborMarketTab() {
  ...
  const fetchLaborData = async (fast = false) => {
    try {
      ...
      setApiStatus(result.metadata?.apiStatus || null);
  ...
  const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: any[]; label?: string }) => {
    if (active && payload && payload.length) {
      ...
      const formattedDate = label ? new Date(label).toLocaleDateString() : 'Unknown';
      return (
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        {dataSource === 'alpha_vantage_tiingo_real_data' ? (
          <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
            <div className="text-xs text-green-700 font-semibold mb-1">✅ REAL DATA SOURCE</div>
            <div className="text-xs text-green-600">
              This data is fetched from Alpha Vantage Economic Data API - providing real labor market indicators. 
              API Status: Alpha Vantage {apiStatus?.alphaVantage ? '✅' : '❌'}, Tiingo {apiStatus?.tiingo ? '✅' : '❌'}
            </div>
          </div>
        ) : (
      ...
      {dataSource === 'alpha_vantage_tiingo_real_data' ? (
        <div className="bg-green-50 border border-green-300 rounded-lg p-4">
          ...
          <div className="text-xs text-green-600 space-y-1">
            <div>• Data fetched from Alpha Vantage Economic Data API</div>
            <div>• Provides authentic unemployment, jobless claims, and employment statistics</div>
            <div>• API Status: Alpha Vantage {apiStatus?.alphaVantage ? '✅ Connected' : '❌ Failed'}, Tiingo {apiStatus?.tiingo ? '✅ Connected' : '❌ Failed'}</div>
