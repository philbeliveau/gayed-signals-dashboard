...
import { BookOpen, FileText, Download, X, ChevronRight } from 'lucide-react';
...
export default function SignalMethodologyCard({
  ...
  useEffect(() => {
    if (showMethodologyModal || showPDFModal) {
  ...
  }, [showMethodologyModal, showPDFModal]);
  ...
  const methodologyModal = showMethodologyModal && mounted ? createPortal(
    <div
    ...
    >
      <div className="bg-theme-card border-2 border-theme-border rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
        {/* Modal Header */}
  ...
  const pdfModal = showPDFModal && selectedPaper && selectedPaper.blobUrl && mounted ? createPortal(
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[110] flex flex-col animate-in fade-in duration-200">
      {/* Modal Header */}
