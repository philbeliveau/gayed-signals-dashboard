...
export class DebateOrchestrator {
  ...
  private async startDebateRound(
    ...
    // Generate debateArguments from each agent's investigation
    for (const agentType of session.participatingAgents) {
      ...
      if (investigation) {
        ...
        round.debateArguments.push(argument);
      }
    }

    // Detect evidence conflicts
    round.evidenceConflicts = await this.detectEvidenceConflicts(round.debateArguments);

    // Calculate interim consensus
    round.interimConsensus = await this.calculateInterimConsensus(round.debateArguments);
   ...
   */
  private async detectEvidenceConflicts(debateArguments: DebateArgument[]): Promise<EvidenceConflict[]> {
    const conflicts: EvidenceConflict[] = [];
    
    // Compare evidence across different agent positions
    const supportingArgs = debateArguments.filter(arg => arg.position === 'SUPPORTS');
    const refutingArgs = debateArguments.filter(arg => arg.position === 'REFUTES');
    ...
    const temporalConflicts = this.detectTemporalConflicts(debateArguments);
   ...
   */
  private detectTemporalConflicts(debateArguments: DebateArgument[]): EvidenceConflict[] {
    const conflicts: EvidenceConflict[] = [];
  ...
  /**
   * Calculate interim consensus from current round debateArguments
   */
  private async calculateInterimConsensus(debateArguments: DebateArgument[]): Promise<ConsensusResult> {
    const supportingVotes = debateArguments.filter(arg => arg.position === 'SUPPORTS');
    const refutingVotes = debateArguments.filter(arg => arg.position === 'REFUTES');
    const neutralVotes = debateArguments.filter(arg => arg.position === 'NEUTRAL');
    ...
    return {
      ...
      claimId: debateArguments[0]?.evidence[0]?.url || 'unknown',
      finalVeracity,
      confidenceScore: agreementLevel,
      consensusMethod,
      supportingEvidence: debateArguments.flatMap(arg => arg.evidence),
      reasoningSummary: this.generateReasoningSummary(debateArguments, finalVeracity),
      participatingAgents: debateArguments.map(arg => arg.agentType),
      agreementLevel,
      minorityOpinions: this.extractMinorityOpinions(debateArguments, finalVeracity),
      createdAt: new Date()
    };
  }
   ...
   */
  private generateReasoningSummary(debateArguments: DebateArgument[], veracity: VeracityLevel): string {
    const agentCount = debateArguments.length;
    const supportCount = debateArguments.filter(arg => arg.position === 'SUPPORTS').length;
    const refuteCount = debateArguments.filter(arg => arg.position === 'REFUTES').length;
    const neutralCount = debateArguments.filter(arg => arg.position === 'NEUTRAL').length;
   ...
   */
  private extractMinorityOpinions(debateArguments: DebateArgument[], majorityVeracity: VeracityLevel): DebateArgument[] {
    const majorityPosition = majorityVeracity === 'VERIFIED_TRUE' ? 'SUPPORTS' : 
                           majorityVeracity === 'VERIFIED_FALSE' ? 'REFUTES' : 'NEUTRAL';
    
    return debateArguments.filter(arg => arg.position !== majorityPosition);
  }
